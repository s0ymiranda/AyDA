...
template <typename T>
std::unordered_set<Designar::Node<Designar::Graph<T>>*> get_set(const Designar::Graph<T>& g, Designar::Node<Designar::Graph<T>>* p, int L);
...
int main()
{
    ...
    auto start_node = hexagonal_grid.search_node([&cow_position] (auto node)
                        {
                            return node->get_info() == cow_position;
                        });
...
std::unordered_set<Designar::Node<Designar::Graph<T>>*> get_set(const Designar::Graph<T>& g, Designar::Node<Designar::Graph<T>>* p, int L)
{
    std::unordered_set<Designar::Node<Designar::Graph<T>>*> saborized; // Conjunto para almacenar los pastos saborizados
    std::unordered_map<Designar::Node<Designar::Graph<T>>*, int> distance; // Mapa para almacenar las distancias desde el punto p

    std::queue<std::pair<Designar::Node<Designar::Graph<T>>*, int>> q;
